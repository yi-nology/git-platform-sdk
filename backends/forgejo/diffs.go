...
func (p *Provider) GetCRDiff(ctx context.Context, owner, repo, number string) (*provider.MergeDiff, error) {
	n, err := backendutil.ParsePRNumber64(provider.PlatformForgejo, "GetCRDiff", number)
	if err != nil {
		return nil, err
	}
	diffBytes, _, err := p.client.GetPullRequestDiff(owner, repo, n, forgejo.PullRequestDiffOptions{})
	if err != nil {
...
func (p *Provider) GetCRFiles(ctx context.Context, owner, repo, number string) ([]*provider.ChangedFile, error) {
	n, err := backendutil.ParsePRNumber64(provider.PlatformForgejo, "GetCRFiles", number)
	if err != nil {
		return nil, err
	}
	changedFiles, _, err := p.client.ListPullRequestFiles(owner, repo, n, forgejo.ListPullRequestFilesOptions{
...
func (p *Provider) CreateNote(ctx context.Context, owner, repo, number, body string) (string, error) {
	n, err := backendutil.ParsePRNumber64(provider.PlatformForgejo, "CreateNote", number)
	if err != nil {
		return "", err
	}
...
// as Forgejo's delete-comment endpoint does not take it.
func (p *Provider) DeleteNote(ctx context.Context, owner, repo, number, noteID string) error {
	if _, err := backendutil.ParsePRNumber64(provider.PlatformForgejo, "DeleteNote", number); err != nil {
		return err
	}
...
func (p *Provider) CreateDiscussion(ctx context.Context, owner, repo, number string, opts provider.DiscussionOptions) (string, error) {
	n, err := backendutil.ParsePRNumber64(provider.PlatformForgejo, "CreateDiscussion", number)
	if err != nil {
		return "", err
	}
	comment, _, err := p.client.CreateIssueComment(owner, repo, n, forgejo.CreateIssueCommentOption{Body: opts.Body})
	if err != nil {
