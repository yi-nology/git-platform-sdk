...
func (p *Provider) UpdateIssue(ctx context.Context, owner, repo string, number int, opts provider.UpdateIssueOptions) (*provider.Issue, error) {
	...
	if len(opts.Labels) > 0 {
		...
		if _, _, err := p.client.ReplaceIssueLabels(owner, repo, int64(number), forgejo.IssueLabelsOption{Labels: ids}); err != nil {
...
// EditIssueOption. Forgejo always serializes Title (no omitempty), so when the
...
func (p *Provider) buildEditIssueOption(op, owner, repo string, number int, opts provider.UpdateIssueOptions) (forgejo.EditIssueOption, error) {
...
func (p *Provider) CloseIssue(ctx context.Context, owner, repo string, number int) (*provider.Issue, error) {
	return p.setIssueState(owner, repo, number, forgejo.StateClosed, "CloseIssue")
}
...
func (p *Provider) ReopenIssue(ctx context.Context, owner, repo string, number int) (*provider.Issue, error) {
	return p.setIssueState(owner, repo, number, forgejo.StateOpen, "ReopenIssue")
}
...
func (p *Provider) setIssueState(owner, repo string, number int, state forgejo.StateType, op string) (*provider.Issue, error) {
	current, _, err := p.client.GetIssue(owner, repo, int64(number))
	if err != nil {
		return nil, provider.Wrap(provider.PlatformForgejo, op, err)
	}
	issue, _, err := p.client.EditIssue(owner, repo, int64(number), forgejo.EditIssueOption{Title: current.Title, State: &state})
	if err != nil {
		return nil, provider.Wrap(provider.PlatformForgejo, op, err)
	}
	return convertIssue(issue), nil
...
func (p *Provider) AddIssueLabels(ctx context.Context, owner, repo string, number int, labels []string) error {
	...
	if _, _, err := p.client.AddIssueLabels(owner, repo, int64(number), forgejo.IssueLabelsOption{Labels: ids}); err != nil {
