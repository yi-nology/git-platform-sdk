...
// paginated resolver with a per-provider TTL cache. Forgejo's update and
...
// unexported helper's name. The forgejo SDK's label methods accept no
...
func (p *Provider) resolveLabelID(op, owner, repo, name string) (int64, error) {
	...
	if err != nil {
		if errors.Is(err, backendutil.ErrLabelScanLimit) {
			...
			return 0, provider.Wrap(provider.PlatformForgejo, op, backendutil.NewScanLimitError(msg))
		}
