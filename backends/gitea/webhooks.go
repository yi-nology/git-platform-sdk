...
func (p *Provider) CreateWebhook(ctx context.Context, opts provider.CreateWebhookOptions) (*provider.PlatformWebhook, error) {
	...
	if err != nil {
		return nil, provider.Wrap(provider.PlatformGitea, "CreateWebhook", err)
	}
...
func (p *Provider) ValidateWebhookSignature(r *http.Request, secret string) error {
	...
	if sig == "" {
		return provider.Wrapf(provider.PlatformGitea, "ValidateWebhookSignature", "missing X-Gitea-Signature header")
	}
	...
	if !hmac.Equal([]byte(sig), []byte(expected)) {
		return provider.Wrapf(provider.PlatformGitea, "ValidateWebhookSignature", "invalid webhook signature")
	}
	return nil
}
